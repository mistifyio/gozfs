...
func (s *internal) TestCreateFS() {
	...
	s.EqualError(create(s.pool+"/"+strings.Repeat("1", 249), dmuZFS, nil), einval)
...
func (s *internal) TestCreateVOL() {
	...
	tests := []struct {
		...
		{s.pool + "/" + strings.Repeat("a", 255), nil, einval},
		{s.pool + "/~1", nil, einval},
...
func unmount(ds string) error {
	cmd := exec.Command("sudo", "zfs", "unmount", ds)
	cmd.Stderr = os.Stderr
	return cmd.Run()
}

func unhold(tag, snapshot string) error {
	cmd := exec.Command("sudo", "zfs", "release", tag, snapshot)
	cmd.Stderr = os.Stderr
	return cmd.Run()
}
